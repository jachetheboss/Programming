...
wormholes = 6
wormhole_points = [(0,0), (1,0), (2,0), (3,0), (4,0), (1,1)]
...
for i in range(pair_combinations):
    ...
    for point in wormhole_points:
        ...
        y = out[1]
        stops = 2
        rightmost = max(y_coor_matches[y])
        while x != rightmost and stops <= 2 * wormholes: ####### faulty logic
            ...
            if x == rightmost:
                break
            print("y_coor_matches[y]", y_coor_matches[y])
            stops += 2
            print(stops)
        if stops > 2 * wormholes and x != rightmost:
            solutions += 1
            stops = 0
            break
    print(pairs)
